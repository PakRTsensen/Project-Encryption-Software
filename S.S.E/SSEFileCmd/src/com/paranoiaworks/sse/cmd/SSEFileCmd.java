...
public class SSEFileCmd {
	...
	public static void main(String[] args) {
		...
		while(true)
		{
			...
			try {
				Encryptor encryptor = new Encryptor(password.toCharArray(), algorithmCode, Encryptor.PURPOSE_FILE_ENCRYPTION, true);
				
				// enc/dec process
				if(!inputFile.isEncrypted())
					errors = (int)encryptor.zipAndEncryptFile(inputFile, compress, mh, null, logs);
				else
					errors = (int)encryptor.unzipAndDecryptFile(new CryptFile(inputFilePath), mh, null, logs);
